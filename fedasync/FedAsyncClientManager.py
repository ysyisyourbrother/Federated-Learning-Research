...
class FedAsyncClientManager(ClientManager):
    ...
    def handle_message_receive_model_from_server(self, msg_params):
        ...
        print(f"########### client {client_index:.3f} delay {delay_time}s")

        self.send_model_to_server(0, weights, local_sample_num, 
                                  self.client_model_round, client_index)

    def send_model_to_server(self, receive_id, weights, local_sample_num, client_model_round, client_index):
